...
def main():
    ...
    df_baseball_1 = data_process.clean_df(df_baseball_1)
    df_baseball_1.rename(columns={"winner_home_or_away": "HomeTeamWins"}, inplace=True)
    ...
    df_hw4_completed = df_hw4_completed.sort_values(by="Importance", ascending=False)
    df_hw4_completed.to_csv("importance.csv")
    # Continous & Continous
    predictors_column = []
    pearson_r = []
    ...
    cont_cont_correlation = pd.DataFrame(
        {
            ...
            "Logistic Regression Plot": cont_cont_linear_plot_link,
        }
    )
